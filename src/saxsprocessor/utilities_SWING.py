...
def view_position_grid_from_single_h5_map(
    h5file: str,
    basler_coords: tuple = (186, 309),
    basler_calibration: tuple = (3.7, 3.7),
    marker_size: float = 3,
    ...
    """
    Visualize all acquisition positions from a single SWING h5 map file on Basler reference image.
    
    Simplified version for h5 files that contain the complete 2D map (all frames).
    
    Parameters
    ----------
    h5file : str
        Path to h5 file containing the complete map (2D array: lines × columns)
    basler_coords : tuple
        Reference position (x, y) in pixels on Basler image
    basler_calibration : tuple
        Calibration in µm/pixel (x, z)
    ...
    dict
        Dictionary containing:
        - 'positions_x': array of x positions in pixels
        - 'positions_z': array of z positions in pixels
        - 'total_frames': total number of frames processed
        - 'figure': matplotlib figure object (if plot=True)
        - 'stats': dictionary with position statistics
        - 'reference_image': Basler reference image
        - 'samplename': sample name
    """
    
    if verbose:
        print(f"{'='*60}")
        print(f"VIEW POSITION GRID FROM SINGLE H5 MAP FILE")
        print(f"{'='*60}")
    ...
    if verbose:
        print(f"Map geometry: {nb_lines} lines × {nb_columns} columns")
        print(f"Reference position: X={swing_file.position_x_start[0]:.4f} mm, Z={swing_file.position_z_start[0]:.4f} mm")
        print(f"Basler calibration: {basler_calibration[0]} µm/pixel, {basler_calibration[1]} µm/pixel")
        print(f"Basler reference coords: {basler_coords}")
    
    # Reference position in mm - ensure scalar values
    x_start = np.atleast_1d(swing_file.position_x_start)[0]
    z_start = np.atleast_1d(swing_file.position_z_start)[0]
    
    # Extract step values as scalars
    step_x = np.atleast_1d(swing_file.step_x)[0]
    step_z = np.atleast_1d(swing_file.step_z)[0]
    
    x_ref_mm = x_start
    z_ref_mm = z_start
    
    if verbose:
        print(f"x_start (scalar): {x_start}, type: {type(x_start)}")
        print(f"z_start (scalar): {z_start}, type: {type(z_start)}")
        print(f"step_x (scalar): {step_x}, type: {type(step_x)}")
        print(f"step_z (scalar): {step_z}, type: {type(step_z)}")
    
    # Calculate position arrays (same as in plot_transmission_map_from_singleh5)
    x_array = np.array([x_start + j * step_x for j in range(nb_columns)])
    z_array = np.array([z_start + i * step_z for i in range(nb_lines)])
    
    if verbose:
        print(f"x_array: {x_array}")
        print(f"z_array: {z_array}")
        print(f"x_array shape: {x_array.shape}, z_array shape: {z_array.shape}")
    
    # Calculate positions in pixels for all grid points
    positions_x = []
    positions_z = []
    
    for i in range(nb_lines):
        for j in range(nb_columns):
            x_mm = float(x_array[j])
            z_mm = float(z_array[i])
            
            # Calculate displacement from reference (mm)
            delta_x_mm = x_mm - x_ref_mm
            delta_z_mm = z_mm - z_ref_mm
            
            # Convert to pixels: step_x/step_z are already in µm (multiplied by 1000 in filereader)
            # so we just divide by calibration (µm/pixel)
            delta_x_pixels = (delta_x_mm * 1000) / basler_calibration[0]
            delta_z_pixels = (delta_z_mm * 1000) / basler_calibration[1]
            
            # Calculate position in pixels on Basler image
            x_pixel = basler_coords[0] + delta_x_pixels
            z_pixel = basler_coords[1] + delta_z_pixels
            
            positions_x.append(x_pixel)
            positions_z.append(z_pixel)
    
    if verbose:
        print(f"First few positions: {list(zip(positions_x[:5], positions_z[:5]))}")
    ...
    # Calculate statistics
    stats = {
        'x_min': positions_x.min(),
        'x_max': positions_x.max(),
        'x_range': positions_x.max() - positions_x.min(),
        'z_min': positions_z.min(),
        'z_max': positions_z.max(),
    ...
    if verbose:
        ...
        print(f"STATISTICS")
        print(f"{'='*60}")
        print(f"Total positions: {stats['total_points']} ({nb_lines}×{nb_columns})")
        print(f"Position X: min={stats['x_min']}, max={stats['x_max']}, range={stats['x_range']} px")
        print(f"Position Z: min={stats['z_min']}, max={stats['z_max']}, range={stats['z_range']} px")
        print(f"{'='*60}")
    ...
    if plot or save_image:
        ...
        # Limit axes to the actual range of positions
        plt.xlim(stats['x_min'] - 10, stats['x_max'] + 10)
        plt.ylim(stats['z_min'] - 10, stats['z_max'] + 10)
        
        plt.title(f'Position Grid Mapping\n'
                  f'{swing_file.samplename}\n'
                  f'{stats["total_points"]} positions ({nb_lines}×{nb_columns})')
        plt.xlabel('Pixel X')
        plt.ylabel('Pixel Z')
        plt.tight_layout()
        ...
        if save_image:
            if output_filename is None:
                output_filename = os.path.join(
                    ...
                    f"position_grid_{swing_file.samplename}.png"
                )
            fig.savefig(output_filename, dpi=300, bbox_inches='tight')
            if verbose:
                print(f"\nImage saved: {output_filename}")
    ...
    return {
        'positions_x': positions_x,
        'positions_z': positions_z,
        'total_frames': nb_lines * nb_columns,
        'figure': fig,
        'stats': stats,
        'reference_image': basler_ref,
        'samplename': swing_file.samplename,
        'geometry': {'nb_lines': nb_lines, 'nb_columns': nb_columns}
    }
