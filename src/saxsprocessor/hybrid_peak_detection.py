...
def detect_peaks_hybrid(q, I, dI=None,
    ...
    d2I = savgol_filter(I_masked, window_length=window_length, 
...
if __name__ == "__main__":
    print("""
    ╔═══════════════════════════════════════════════════════════════════╗
    ║           MÉTHODE HYBRIDE - Guide d'Utilisation                   ║
    ╚═══════════════════════════════════════════════════════════════════╝
    
    La méthode hybride combine :
    ✅ Détection robuste (dérivée seconde)
    ✅ Raffinement précis (SPV)
    
    EXEMPLE D'UTILISATION :
    -----------------------
    
    results = detect_peaks_hybrid(
        q, I,
        nb_peaks=4,
        qmin=0.03,
        qmax=0.125,
        ...
        smooth=True,
        smooth_sigma=2,
        fit_window_width=3,  # Largeur fenêtre en FWHM
        # Visualisation
        verbose=True,
        plot=True
    )
    ...
    print(f"Positions initiales : {results['q_peaks_initial']}")
    print(f"Positions raffinées : {results['q_peaks']}")
    print(f"Incertitudes : {results['q_peaks_std']}")
    print(f"Distances : {results['d_spacings']}")
    ...
    fit_window_width : float, default=3
        Largeur de la fenêtre de fit autour de chaque pic
        en nombre de FWHM estimés.
        • Plus petit (1-2) = fit plus local, moins influencé par voisins
        • Plus grand (3-5) = plus de données, fit plus stable
        
    prominence : float
        Sélectivité de la détection initiale.
        • Plus petit = détecte plus de pics (même faibles)
        • Plus grand = ne garde que les pics majeurs
