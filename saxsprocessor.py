...
class SAXSProcessor:
    ...
    def plot2d_vsq(self, 
        ...
        if mean_angle is not None:
            ...
            # longueur max pour couvrir toute l'image
            r_max = 0.01
            #max(abs(xmin), abs(xmax), abs(ymin), abs(ymax)) * 1.2

            # ligne passant par l'origine
            x_line = np.array([-r_max, r_max])
            y_line = np.tan(theta) * x_line

            # ATTENTION : convention du plot (QZ inversé)
            y_line = -y_line
    ...
    def find_main_orientation(self, qvalue=0.01, threshold=0.05):
        ...
        if I is not None and I.size > 0:
            ...
            '''plt.figure()
            plt.loglog(chi, I, label='Average Radial Profile')
            plt.axvline(angle, linestyle='--', linewidth=2, alpha=0.5, label='q peak')
    ...
    def free_memory(self):
        if hasattr(self.file, "raw_data"):
            del self.file.raw_data
        import gc
        gc.collect()
