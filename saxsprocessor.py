...
class SAXSProcessor:
    ...
    def __init__(self,
        ...
        instrument : str
            'ID02', 'SWING', or 'LGC'
        qvalues : array
            Q values for azimuthal profile extraction (Å⁻¹)
        threshold : float
            Relative tolerance for Q values
        binning : int
            Downsampling factor
        mask : str
    ...
    def export_sasview(self, output_dir=None):
        """
        Export 2D SAXS data in SasView ASCII format:
        Qx  Qy  I  dI  mask
        """

        # 1) Get proper Q grids (must be 2D and physically correct)
        ...
        # 3) Mask: SasView = 0 good, 1 bad
        if self.maskdata is None:
            mask = np.zeros_like(I)
        else:
            mask = (self.maskdata.ravel() > 0).astype(int)

        # Replace masked intensity by NaN
        I = np.where(mask == 1, np.nan, I)

        # 4) Stack columns
        data_out = np.column_stack([qx, qy, I, dI, mask])

        # 5) Output directory
        if output_dir is None:
            output_dir = os.path.join(self.path, "saxs_exports")
        else:
            output_dir = os.path.join(output_dir, "saxs_exports")
        os.makedirs(output_dir, exist_ok=True)

        outfile = os.path.join(output_dir,
                            f"{self.samplename}_B={self.B}mT_Img_{self.file_number}.dat")

        # 6) Header
        header = (
            "# SasView 2D ASCII format\n"
            "# Columns: Qx[1/A], Qy[1/A], I, dI, mask\n"
            f"# Shape: {self.data.shape}\n"
        )

        # 7) Save
        np.savetxt(outfile, data_out, header=header, comments='', fmt="%.7e")

        print(f"[OK] SasView file saved → {outfile}")
