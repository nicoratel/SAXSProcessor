...
class SAXSExperiment:
    ...
    def find_peaks_spv(
        ...
        if plot:
            ...
            plt.figure()
            plt.semilogx(q, y, label=input_attr)
            plt.semilogx(q, model(q, *p0), label='SPV model')
        ...
        if plot:
            ...
            plt.figure()
            plt.semilogx(q, y, label=input_attr)
            plt.semilogx(q, y_model, label=f'SPV model')
    ...
    def find_peaks_spv_batch(
        ...
        if plot:
            plt.figure()
            plt.semilogx(q_init, y_init, label=input_attr)
            plt.semilogx(q_init, y_model_global, label='SPV model')
            plt.plot(q_values_global, model(q_values_global, *popt_global), 'rx', label='Peaks')
            for i in range(n_found_peak):
                popt = popt_global[8 * i : 8 * (i + 1)]
                plt.semilogx(q_init, model(q_init, *popt), '--', label=f'SPV n°{i+1}')
    ...
    def find_peaks_spv_off(
        ...
        if plot:
            ...
            plt.figure()
            plt.semilogx(q, y, label=input_attr)
            plt.semilogx(q, model(q, *p0), label='SPV_off')
            plt.plot(peak_q_value, peak_height, 'rx', label='Peak')
            plt.axvline(peak_l_base, color='g', label='left_base')
            plt.axvline(peak_r_base, color='g', label='right_base')
            plt.title('Initial guess')
        ...
        if plot:
            ...
            plt.figure()
            plt.semilogx(q, y, label=input_attr)
            plt.semilogx(q, y_model, label=f'SPV_off')
            plt.plot(q_value, model(q_value, *popt), 'rx', label='Peak')
            plt.axvline(q_lb, color='g', label='left_base')
    ...
    def find_peaks_spv_off_batch(
        ...
        if plot:
            plt.figure()
            plt.semilogx(q_init, y_init, label=input_attr)
            plt.semilogx(q_init, y_model_global, label='SPV_off model')
            plt.plot(q_values_global, model(q_values_global, *popt_global), 'rx', label='Peaks')
            for i in range(n_found_peak):
                mu, scale, sig1, eta1, _, sig2, eta2, _ = popt_global[8 * i : 8 * (i + 1)]
                plt.semilogx(q_init, model(q_init, mu, scale, sig1, eta1, 0, sig2, eta2, 0), '--', label=f'SPV_off n°{i+1}')
    ...
    def find_peaks_asym2sig(
        ...
        if plot:
            ...
            plt.figure()
            plt.semilogx(q, y, label=input_attr)
            plt.semilogx(q, SAXSTools.asym2sig(q, *p0), label='Asym2Sig')
            plt.plot(peak_q_value, peak_height, 'rx', label='Peak')
            plt.axvline(peak_l_base, color='g', label='left_base')
            plt.axvline(peak_r_base, color='g', label='right_base')
            plt.title('Initial guess')
        ...
        if plot:
            plt.figure()
            plt.semilogx(q, y, label=input_attr)
            plt.semilogx(q, y_model, label=f'Asym2Sig')
    ...
    def find_peaks_asym2sig_batch(
        ...
        if plot:
            plt.figure()
            plt.semilogx(q_init, y_init, label=input_attr)
            plt.semilogx(q_init, y_model_global, label='Asym2Sig model')
            plt.plot(q_values_global, model(q_values_global, *popt_global), 'rx', label='Peaks')
            for i in range(n_found_peak):
                A, mu, w1, w2, w3, _ = popt_global[6 * i : 6 * (i + 1)]
                plt.semilogx(q_init, SAXSTools.asym2sig(q_init, A, mu, w1, w2, w3, np.min(q_init)), '--', label=f'Asym2Sig n°{i+1}')
