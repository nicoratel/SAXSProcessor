...
def _process_one_edf(
    ...
    proc.file_number = i
    # afficher enregistrer les plot pour chaque image si besoin
    # proc.plot2d_vsq()
    ...
    print("file",file)
    print(proc.file_number)
    print(proc.samplename)
    row_data = {
...
def compute_nematic_order_assembly_SWING(
    ...
    S_interp_valid_only = griddata(
        ...
        # --- copie des données ---
    data = S_interp_valid_only.copy()
    cmap2.set_under("cyan")   # couleur STRICTEMENT pour 0
    
    from matplotlib.colors import PowerNorm
    norm = PowerNorm(gamma=1.95, vmin=0, vmax=1)
    ...
    # === CONDITION : plus de lignes que colonnes → on transpose ===
    if data.shape[0] > data.shape[1]:
        # image
        data_plot = data.T
        extent_plot = [z_min_ext, z_max_ext, x_min_ext, x_max_ext]
        ...
        u = np.sin(np.radians(orientation_2d[valid]))  # swap !
        v = np.cos(np.radians(orientation_2d[valid]))

        scale_unit = 'width'
        width = 0.003

        ax2.set_xlabel('Z position (mm)', fontsize=14)
        ax2.set_ylabel('X position (mm)', fontsize=14)

    else:
        # normal
        data_plot = data
        extent_plot = [x_min_ext, x_max_ext, z_min_ext, z_max_ext]
    ...
    plt.savefig(os.path.join(outputpath, f'{safe_samplename}_Cartographytest.png'))
...
def compute_parrallel_vs_perpendicular(proc, mean_angle, qpeaks):
    qpara,Ipara = proc.extract_radial_profile(azimuth=-mean_angle, width=10)
    qperp,Iperp = proc.extract_radial_profile(azimuth=-mean_angle + 90, width=10)

    fig, ax = plt.subplots(figsize=(6, 5))

    ax.loglog(qpara, Ipara, label=f'Azimuth {-mean_angle:.2f}°')
    ax.loglog(qperp, Iperp, label=f'Azimuth {(-mean_angle + 90):.2f}°')

    ax.axvline(qpeaks[0], linestyle='--', linewidth=2, alpha=0.5, label='q peak')
    ...
    qpeak = qpeaks[0]
    I_peak_para = np.interp(qpeak, qpara, Ipara)
    I_peak_perp = np.interp(qpeak, qperp, Iperp)

    ratio = I_peak_perp/I_peak_para

    ax.scatter(qpeak, I_peak_perp, color='red')
    ax.scatter(qpeak, I_peak_para, color='green')
    ax.set_title(f'Intensité parallèle et perpendiculaire pour width=10° | ratio={ratio:.2f}')
    output_dir1 = os.path.join(proc.path, 'Ratio_intensité')
    os.makedirs(output_dir1, exist_ok=True)
    figname1 = os.path.join(output_dir1, f'{proc.samplename}_Img_{proc.file_number}.png')
    plt.savefig(figname1, dpi=300, bbox_inches='tight')
    plt.close(fig)
    return ratio

def plot_transmission_map_from_singleh5(h5file, proc, plot = True):
    ...
    transmission_array = (mi8a / mi5) /(3.46e-6 / 1.203e-8 )
    ...
    ax1.set_xlabel('X position (mm)', fontsize=14)
    ax1.set_ylabel('Z position (mm)', fontsize=14)
    ax1.set_title('Transmission map', fontsize=16)
    ax1.invert_yaxis()

    cbar1 = plt.colorbar(im1, ax=ax1)
    cbar1.set_label('Transmission', rotation=270, labelpad=20, fontsize=14)

    plt.tight_layout()

    output_dir1 = os.path.join(proc.path, 'saxs_trans')
    os.makedirs(output_dir1, exist_ok=True)
    ...
    ax2.set_xlabel('X position (mm)', fontsize=14)
    ax2.set_ylabel('Z position (mm)', fontsize=14)
    ax2.set_title('Transmission map', fontsize=16)
    ax2.invert_yaxis()

    cbar2 = plt.colorbar(im2, ax=ax2)
    cbar2.set_label('Transmission', rotation=270, labelpad=20, fontsize=14)

    plt.tight_layout()

    output_dir2 = os.path.join(proc.path, 'saxs_trans', 'echelle_nn_contrainte')
    os.makedirs(output_dir2, exist_ok=True)

    figname2 = os.path.join(output_dir2, f'{proc.samplename}_Img_{proc.file_number}.png')
...
def global_analysis(
    ...
    for file in os.listdir(h5path): # only if nxs or h5 files
        if file.endswith('.h5') or file.endswith('.nxs'):
            filename = file
            filepath =  os.path.join(h5path, filename)
            print("TYPE file:", type(file))
            print(f"\nProcessing file: {filename}")
            print("chemin", os.path.join(h5path, filename))
            status = "success"
            error_message = ""
            ...
            try:
                ...
                if L is None or radius is None:
                    diameter, radius_pd, length, L_pd, B, reference_file = get_all_params(
                        excel_file, bg_path, file
                    )
                ...
                if reference_file is None:
                    raise ValueError("reference_file is None → skipping")
                
                print(f"with reference file: {reference_file}")
                ...
                plot_transmission_map_from_singleh5(filepath, proc, plot=False)
                
                proc.plot2d_vsq(plot=False)
                ...
                qvalues = results_dict['peaklist']
                distances = results_dict['distances']
                
                results.update({
                    'q_peak (A⁻¹)': qvalues,
                    'distance (A)': distances[0] if distances is not None else None,
                })
                ...
                S, mean_orientation, res = compute_nematic_parameter(
                    ...
                    L=length * 10,
                    radius_pd=radius_pd * 0.01,
                    L_pd=L_pd * 0.01,
                    plot=False,
                    apply_mirror=None,
                    verbose=True
                )
                ratio = compute_parrallel_vs_perpendicular(proc, mean_orientation, qvalues)
                # ---------------- SAVE RESULTS ----------------
                results.update({
                ...
                fileC = h5File_SWING(testfile, mean = False) # we do not average data contained in this file, we force linescan (step_z=0)
                filer = h5File_SWING(reference, mean = False)
                
                
                filer.convert2edf(outputdir=edfpathr)
                fileC.convert2edf(outputdir=edfpath)
                ...
                def parse_img(filename):
                    match = re.search(r'img_(\d+)_(\d+)', filename, re.IGNORECASE)
                    if match:
                        return int(match.group(1)), int(match.group(2))
                ...
                print(sorted_files)
                print(sorted_filesr)
                milieu = int(filer.data.shape[1]*filer.data.shape[0]*1/2 + filer.data.shape[0]*1/2)
                print("Numero de l'image prise au sein de la reference", milieu)
                refmil = sorted_filesr[milieu]
                print("TYPE file:", type(file))
                
                if fileC.data.shape[1] > 2 and fileC.data.shape[0] > 2:
                    R2_2d = compute_nematic_order_assembly_SWING(
                        ...
                        apply_mirror=False,
                        verbose=True,
                        nb_ligne=fileC.data.shape[1],  # A integrer proprement dans la fonction
                        nb_colonne=fileC.data.shape[0]
                        )
                    import gc
                    gc.collect()
                    gc.collect()
            ...
            if results['status'] == "success":
                import gc
                print("effacement de la mem")
                proc.free_memory()
                del proc, fileC, filer, results_dict, get_results
                proc = None
                gc.collect()

            print(f"→ Saved result for {file} (status: {status})")
...
'''
        if file.endswith('.h5') or file.endswith('.nxs'):
            ...
            if L is None or radius is None:
                diameter, radius_pd, length, L_pd, B, reference_file  = get_all_params(excel_file, bg_path, file) # here specify the measurement name
                print("ref",diameter, radius_pd, length,L_pd,  reference_file)
                if reference_file is not None : # dont compute the S for a reference file
                    ...
                    # plot transmission
                    plot_transmission_map_from_singleh5(os.path.join(h5path, file), proc, plot=False)
                    ...
                    qvalues = results_dict['peaklist']
                    distances = results_dict['distances']
                    print("distances", qvalues)
                    # Compute global nematic order parameter
                    S, mean_orientation, res = compute_nematic_parameter(
                        proc,
                        threshold=threshold,
                        radius= diameter * 10/2,
                    ...
                    testfile = Path(os.path.join(h5path, file))
                    edfpath = testfile.parent / "cartographie" / testfile.stem
                    edfpath.mkdir(parents=True, exist_ok=True)
                    edfpath = str(edfpath)
                    ...
                    milieu = int(filer.data.shape[1]*filer.data.shape[0]*1/2 + filer.data.shape[0]*1/2)
                    print(filer.data.shape[1], milieu)
                    refmil = sorted_filesr[milieu]
                    
                    if file.data.shape[1] > 2 and file.data.shape[0]:
                    ...
                    'distance (A)': distances[0], # array of distances
                    'mean_orientation (°)': mean_orientation, # array of mean orientations
