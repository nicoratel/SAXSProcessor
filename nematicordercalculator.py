...
class BatchNematic:
    ...
    def __init__(self, 
        ...
        self.qvalues = qvalues if qvalues is not None else [0.034]
    ...
    def process_all(self, save_profiles=True, plot=False, apply_mirror=False):
        ...
        if len(results) > 0:
            ...
            print("Summary Statistics")
            print(f"{'=' * 60}")
            print(f"Total files processed: {len(self.h5_filelist)}")
            print(f"Successful fits: {len(results)}")
            print(f"Failed fits: {len(self.h5_filelist) * len(self.qvalues) - len(results)}")
            
            if len(results) > 0:
                print(f"\nNematic Order Parameter (S) statistics:")
                for q in self.qvalues:
                    q_data = df[df['q (Å⁻¹)'] == q]['S']
                    if len(q_data) > 0:
                        print(f"  q = {q:.4f} Å⁻¹:")
                        print(f"    Mean S = {q_data.mean():.4f} ± {q_data.std():.4f}")
                        print(f"    Range: [{q_data.min():.4f}, {q_data.max():.4f}]")
