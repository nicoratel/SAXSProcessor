...
class NematicOrderCalculator:
    ...
    def fit_azimuthal_profile(self, theta_exp, I_exp, 
        ...
        plt.axvline(mean_angle,color='red', linestyle='--', linewidth=2, alpha=0.5, label='mean_angle')
        plt.tight_layout()
        if processor is not None:
        
            try: #position x
                figname=os.path.join(os.path.join(processor.path,'azimuthal_profiles'),f'{processor.samplename}_B={processor.B}_Img{processor.file_number}_x={float(processor.x):.2f}_z={float(processor.z):.2f}_nematic_determination.png')
            except:
...
if SASMODELS_AVAILABLE:
    class CylinderFormFactor:
        ...
        def __init__(self,
            ...
            self.Qx = self.Qx.astype(np.float32)
            self.Qy = self.Qy.astype(np.float32)
            self.Qz = self.Qz.astype(np.float32)
