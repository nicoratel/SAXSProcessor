...
try:
    ...
    WAXS=False
    print('waxsprocessor.py not found')
try:
...
class NematicOrderCalculator:
    ...
    def fit_azimuthal_profile(self, theta_exp, I_exp, 
        ...
        bounds = ([0, 0, target-2, -np.inf, -np.inf],
                 [np.inf, 500, target+2, np.inf, np.inf])
        ...
        if plot:
            ...
            if processor is not None:
                try:
                    if processor.instrument != 'ID02':
                        figname=f'{processor.path}/{processor.samplename}_B={processor.B}_x={processor.x:.2f}_z={processor.z}_Img{processor.file_number}_nematic_determination.png'
                    else:
                        figname=f'{processor.path}/{processor.samplename}_B={processor.B}_Img{processor.file_number}_nematic_determination.png'
                    plt.savefig(figname)
                except:
                    pass
