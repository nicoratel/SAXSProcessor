...
class h5File_SWING:
    ...
    def _extract_scatteringdata(self):
        with h5py.File(self.file, "r") as f:
            ...
            target = group + '/scan_data/eiger_image'
            #data_raw= f[target]
            #print("data", data_raw, data_raw.shape)
    ...
    def _extract_scatteringdata(self):
        with h5py.File(self.file, "r") as f:
            ...
            data_raw = f[target]  # ⚠️ PAS de np.array
            self.raw_data = data_raw
            if (len(data_raw.shape) == 3) and self.mean:
                data = np.mean(data_raw, axis=0)

            elif (len(data_raw.shape) == 4) and self.mean:
                # ⚠️ ici on force quand même le chargement
                data = np.mean(np.mean(data_raw, axis=0), axis=0)

            else:
                # ⚠️ DANGER : ici tu renvoyais tout
                data = data_raw[()]  # ou mieux → slice plus tard
